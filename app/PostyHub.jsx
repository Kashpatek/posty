...
// ═══ STORAGE ═══
function load(k){try{if(typeof window!=="undefined"&&window.storage){return window.storage.get(k).then(r=>r?JSON.parse(r.value):null).catch(()=>null);}if(typeof localStorage!=="undefined"){const v=localStorage.getItem(k);return Promise.resolve(v?JSON.parse(v):null);}return Promise.resolve(null);}catch{return Promise.resolve(null);}}
function save(k,v){try{const s=JSON.stringify(v);if(typeof window!=="undefined"&&window.storage)window.storage.set(k,s).catch(()=>{});if(typeof localStorage!=="undefined")localStorage.setItem(k,s);}catch{}}
